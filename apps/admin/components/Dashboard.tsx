...
import {
  ...
  CircleDollarSign, ClipboardList, FileCheck2, FileText, Home, Landmark, LayoutDashboard, LogOut, MapPinned,
  Menu, MessageSquare, Moon, MoreHorizontal, Search, Settings, ShieldCheck, Sun, UsersRound, UserRound, X,
} from 'lucide-react';

const rows = [
  { no: 'SS-HARI-2026-004281', name: 'Md. Rafiqul Islam', service: 'Citizen Certificate', date: '18 September 2026', status: 'Awaiting Approval' },
  { no: 'SS-RANI-2026-004279', name: 'Ms. Sharmin Akter', service: 'Heir Certificate', date: '18 September 2026', status: 'Verification in Progress' },
  { no: 'TL-HARI-2026-000198', name: 'M/S Rahman Store', service: 'Trade License', date: '17 September 2026', status: 'Ready for Issue' },
  { no: 'SS-RANI-2026-004275', name: 'Md. Abdul Karim', service: 'Character Certificate', date: '17 September 2026', status: 'Approved' },
];
...
const menuItems: MenuItem[] = [
  ...
  { key: 'applications', label: 'Service Applications', icon: ClipboardList, badge: '12', children: [{ key: 'certificate-applications', label: 'Certificate Applications' }, { key: 'application-review', label: 'Application Review' }] },
...
const moduleContent: Record<string, { title: string; subtitle: string; action: string; columns: string[]; values: string[][] }> = {
  'citizen-list': { title: 'Citizen List', subtitle: 'View and verify registered citizens', action: 'New Citizen', columns: ['Name', 'Mobile', 'Union', 'Status'], values: [['Md. Rafiqul Islam', '01712345678', 'Union 01', 'Verified'], ['Ms. Sharmin Akter', '01812345678', 'Union 02', 'Verification Pending']] },
  'citizen-verify': { title: 'Citizen Verification', subtitle: 'Review citizen registration applications', action: 'New Verification', columns: ['Applicant', 'NID', 'Submission Date', 'Status'], values: [['Md. Abdul Karim', '1990••••••12', '18 September 2026', 'Pending']] },
  applications: { title: 'Service Applications', subtitle: 'Review and approve certificate applications', action: 'View Applications', columns: ['Tracking Number', 'Applicant', 'Service', 'Status'], values: rows.slice(0, 3).map((r) => [r.no, r.name, r.service, r.status]) },
  'certificate-applications': { title: 'Certificate Applications', subtitle: 'List of all certificate applications', action: 'New Application', columns: ['Number', 'Applicant', 'Service Name', 'Status'], values: [['SS-HARI-2026-004281', 'Md. Rafiqul Islam', 'Citizen Certificate', 'Awaiting Approval']] },
  'application-review': { title: 'Application Review', subtitle: 'Applications awaiting initial review', action: 'Start Review', columns: ['Number', 'Service', 'Submission Date', 'Status'], values: [['SS-RANI-2026-004279', 'Heir Certificate', '18 September 2026', 'Verification in Progress']] },
  'trade-license': { title: 'Trade License', subtitle: 'Trade license application, issue & renewal management', action: 'New License', columns: ['License Number', 'Organization', 'Owner', 'Status'], values: [['TL-HARI-2026-000198', 'M/S Rahman Store', 'Md. Rahman', 'Ready for Issue']] },
  'holding-assessment': { title: 'Holding Tax Assessment', subtitle: 'Tax determination and assessment by holding', action: 'Assess Tax', columns: ['Holding No.', 'Head of Household', 'Annual Value', 'Tax'], values: [['123/45', 'Md. Rafiqul Islam', '৳ 25,000', '৳ 1,850'], ['123/46', 'Md. Abdul Karim', '৳ 18,000', '৳ 1,200']] },
  'non-holding-assessment': { title: 'Non-Holding Tax Assessment', subtitle: 'Tax assessment for non-holding citizens', action: 'Assess Tax', columns: ['Citizen', 'Ward', 'Fiscal Year', 'Tax'], values: [['Md. Saiful Islam', '03', '2025-2026', '৳ 500']] },
  'holding-payment': { title: 'Holding Tax Payment', subtitle: 'Holding tax collection, receipt & arrears management', action: 'Add Payment', columns: ['Receipt No.', 'Holding', 'Amount', 'Date'], values: [['RC-2026-00128', '123/45', '৳ 650', '18 September 2026']] },
  'non-holding-payment': { title: 'Non-Holding Tax Payment', subtitle: 'Verify non-holding tax payments', action: 'Add Payment', columns: ['Receipt No.', 'Citizen', 'Amount', 'Status'], values: [['RC-2026-00127', 'Md. Saiful Islam', '৳ 500', 'Paid']] },
  ...
  'profile-create': { title: 'Profile Creation', subtitle: 'Create new citizen and family profiles', action: 'Add Profile', columns: ['Profile No.', 'Name', 'Type', 'Status'], values: [['PR-2026-0042', 'Md. Rafiqul Islam', 'Family', 'Active']] },
  area: { title: 'Area', subtitle: 'Manage geographic structure from district to village', action: 'New Area', columns: ['Name', 'Type', 'Parent', 'Status'], values: [['Union 01', 'Union', 'District 01', 'Active'], ['Union 02', 'Union', 'District 02', 'Active']] },
  'service-reports': { title: 'Service Reports', subtitle: 'All service and application reports', action: 'Export Report', columns: ['Report', 'Period', 'Records', 'Action'], values: [['Service Application Summary', 'September 2026', '34', 'Download']] },
  'holding-report': { title: 'Holding Tax Report', subtitle: 'Holding tax collection & arrears report', action: 'Download PDF', columns: ['Report', 'Holding', 'Collected', 'Arrears'], values: [['Monthly Collection Report', '2,846', '৳ 42,850', '৳ 18,200']] },
  'non-holding-report': { title: 'Non-Holding Tax Report', subtitle: 'Non-holding tax collection report', action: 'Download Report', columns: ['Report', 'Citizen', 'Collected', 'Arrears'], values: [['September Report', '432', '৳ 12,400', '৳ 3,200']] },
  'license-report': { title: 'Trade License Report', subtitle: 'License issue & renewal report', action: 'Download Report', columns: ['Report', 'License', 'New', 'Renewed'], values: [['Monthly License Report', '476', '32', '87']] },
...
export default function Dashboard() {
  ...
  const [expanded, setExpanded] = useState<string[]>(['tax-payment']);
  ...
  useEffect(() => {
    ...
    if (urlToken) {
      localStorage.setItem('accessToken', decodeURIComponent(urlToken));
  ...
  const logout = () => { localStorage.removeItem('accessToken'); localStorage.removeItem('user'); window.location.href = 'http://localhost:3000/login'; };
  ...
  return <div className={`admin-shell ${dark ? 'admin-dark' : ''}`}>
    ...
    <main className="admin-main">
      ...
      <div className="dash-content">
        {activeMenu === 'dashboard' ? <DashboardHome /> : <ModuleWorkspace active={activeMenu} label={currentLabel} />}
      </div>
    </main>
  </div>;
}
  ...
  return <>
    <div className="dash-heading">
      <div>
        ...
        <p>18 September 2026 · Friday</p>
      </div>
      <button className="export-btn"><ArrowDownRight size={15} /> Export Report</button>
    </div>
    <div className="admin-stats">
      <Stat icon={ClipboardList} value="34" label="Total Applications" change="+12.4%" positive />
      <Stat icon={ClockIcon} value="12" label="Pending" change="Today" />
      <Stat icon={FileCheck2} value="28" label="Certificates Issued" change="+8.2%" positive />
      ...
      <section className="dash-card tasks-card">
        ...
        <div className="task-list">
          <div className="task-item">
            <div className="task-icon yellow"><ClipboardList size={16} /></div>
            <div>
          ...
          <div className="task-item">
            <div className="task-icon green"><CircleDollarSign size={16} /></div>
            <div>
              <strong>Tax Payment Verification</strong>
              <small>Active</small>
            </div>
          </div>
          <div className="task-item">
            <div className="task-icon blue"><FileCheck2 size={16} /></div>
            <div>
...
function ModuleWorkspace({ active, label }: { active: string; label: string }) {
  ...
  return <>
    <div className="dash-heading">
      ...
      <button className="export-btn"><ArrowRight size={15} /> {content.action}</button>
    </div>
    <div className="admin-stats">
      <Stat icon={ClipboardList} value="12" label="Total Records" change="This Month" />
      <Stat icon={ClockIcon} value="04" label="Pending" change="Needs Attention" />
      <Stat icon={ShieldCheck} value="28" label="Completed" change="This Month" positive />
    ...
    <section className="dash-card table-card module-table">
      ...
      <div className="table-scroll">
        <table>
          ...
          <tbody>
            {content.values.map((value, rowIndex) => <tr key={rowIndex}>
              {value.map((cell, index) => <td key={index}>{index === value.length - 1 ? <span className="status approved">{cell}</span> : cell}</td>)}
